...
let rec mcomp type_pairs env t1 t2 =
  ...
  | _ ->
      ...
      with Not_found ->
        ...
        | (_, Tvar _)  ->
            ()
        | (Tarrow (Arrow(l1,_,_), t1, u1, _), Tarrow (Arrow(l2,_,_), t2, u2, _))
...
let unify_alloc_mode a b =
  match
    Types.Alloc_mode.constrain a b,
    Types.Alloc_mode.constrain b a
  with
  | Ok (), Ok () -> ()
  | _ -> raise (Unify [])
  ...
  | _ ->
    ...
    try
      begin match (d1, d2) with
        (Tarrow (Arrow(l1,a1,r1), t1, u1, c1),
         Tarrow (Arrow(l2,a2,r2), t2, u2, c2))
           when
             (l1 = l2 ||
...
let filter_arrow env t ~curry l =
  ...
  match t.desc with
    Tvar _ ->
      ...
      let mret = if curry then Ret_curried else Ret_heap in
      let t' = newty2 lv (Tarrow (Arrow(l,marg,mret), t1, t2, Cok)) in
      link_type t t';
      (marg, t1, mret, t2)
  | Tarrow(Arrow (l', arg, ret), t1, t2, _)
    when (l = l' || !Clflags.classic && l = Nolabel && not (is_optional l))
         && ((ret <> Ret_curried) || curry) ->
      (* FIXME: consider tightening check to (ret = Ret_curred) = curry,
         when better lambda syntax exists. *)
      (arg, t1, ret, t2)
  | _ ->
      raise (Unify [])
        ...
        with Not_found ->
          ...
          match (t1'.desc, t2'.desc) with
            (Tvar _, _) when may_instantiate inst_nongen t1' ->
              ...
              update_scope t1'.scope t2;
              link_type t1' t2
          | (Tarrow (Arrow(l1,a1,r1), t1, u1, _),
             Tarrow (Arrow(l2,a2,r2), t2, u2, _)) when r1 = r2 &&
               (l1 = l2 
