...
let rec constructor_type constr cty =
  ...
  | Cty_signature _ ->
      constr
  | Cty_arrow (l, ty, cty) ->
      Ctype.newty (Tarrow (Arrow (l, Alloc_heap, Alloc_heap),
        ...
        Warnings.mk_lazy
          (fun () ->
             ...
             let meth_type = mk_expected (
               Btype.newgenty (Tarrow(Arrow(Nolabel, Alloc_heap, Alloc_heap),
                                      self_type, ty, Cok))
             ) in
  ...
  | Pcf_initializer expr ->
      ...
      let field =
        lazy begin
          ...
          let meth_type = mk_expected (
            Ctype.newty
              (Tarrow (Arrow (Nolabel, Alloc_heap, Alloc_heap), self_type,
