...
let mk_poly_desc ~loc newtypes body =
  let layouts = List.map snd newtypes in
  let newtypes = List.map fst newtypes in
  let body =
    match attrs_of_layout layouts None with
    | [] -> body
    | attrs ->
      reloc_typ ~loc { body with ptyp_attributes = attrs @ body.ptyp_attributes } in
...
let wrap_type_annotation ~loc newtypes core_type body =
  ...
  (exp, ghtyp(mk_poly_desc ~loc newtypes (Typ.varify_constructors names core_type)))
...
let_binding_body:
  ...
  | let_ident type_constraint EQUAL seq_expr
      { let v = $1 in (* PR#7344 *)
        ...
        let typ = ghtyp ~loc (mk_poly_desc ~loc [] t) in
  ...
  | let_ident COLON typevar_list DOT core_type EQUAL seq_expr
      ...
      { let typloc = ($startpos($3), $endpos($5)) in
        ...
        (ghpat ~loc:patloc
           (Ppat_constraint($1, ghtyp ~loc:typloc (mk_poly_desc ~loc:typloc $3 $5))),
         $7) }
